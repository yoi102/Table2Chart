...
namespace Table2Chart.ColorManipulation
{
    internal static class XyzConverter
    {
        public static Color ToColor(this Xyz xyz)
        {
            ...
            {
                return d > 0.0031308 ? 255.0 * (1.055 * Math.Pow(d, 1.0 / 2.4) - 0.055) : 255.0 * (12.92 * d);
            }

            byte clip(double d)
            {
                return d < 0 ? (byte)0 : d > 255 ? (byte)255 : (byte)Math.Round(d);
            }
        ...
        public static Xyz ToXyz(this Lab lab)
        {
            ...
            {
                return d > LabConstants.eCubedRoot ? d * d * d : (116 * d - 16) / LabConstants.k;
            }
